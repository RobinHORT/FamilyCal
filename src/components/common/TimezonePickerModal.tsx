...
import { Search, X, Check, Globe, MapPin, Clock, ChevronRight, ChevronLeft } from 'lucide-react';
import {
  searchCountries,
...
export const TimezonePickerModal: React.FC<TimezonePickerModalProps> = ({
  ...
  subtitle = 'Choose a country to set your timezone',
}) => {
  ...
  const cityResults = useMemo(() => {
    ...
    if (!q) return selectedCountry.cities;
    return selectedCountry.cities.filter(
      (c) =>
  ...
  const handleCountryClick = (country: CountryInfo) => {
    if (country.isSingleTimezone) {
      onSelectTimezone(country.primaryTimezone);
      onClose();
    ...
    >
      ...
      >
        ...
        <div className="p-5 pb-3 border-b border-gray-100 shrink-0">
          {selectedCountry ? (
            <div>
              <button
                ...
                className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-800 mb-2 transition-colors cursor-pointer"
              >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Select state or region</p>
                  </div>
                </div>
                <button
                  type="button"
          ...
          <div className="relative mt-4">
            ...
            <input
              ...
              placeholder={
                selectedCountry
                  ? `Search region in ${selectedCountry.name}...`
                  : 'Search country...'
              }
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1 divide-y divide-gray-50">
          {selectedCountry ? (
            /* Region/City Selection View for Multi-Timezone Country */
            cityResults.length === 0 ? (
              <div className="py-12 text-center text-gray-400">
                <MapPin className="w-8 h-8 mx-auto mb-2 opacity-40 text-gray-400" />
                <p className="text-xs font-semibold">
                  No regions found matching "{searchQuery}" in {selectedCountry.name}
                </p>
              </div>
            ) : (
              cityResults.map((city, idx) => {
                ...
                const badgeText = `${selectedCountry.iso3}/${city.code}`;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 pr-2">
                      <span
                        className={`px-2 py-1 rounded-xl text-xs font-black tracking-wider shrink-0 shadow-2xs ${
                          isSelected
            ...
            countryResults.length === 0 ? (
              <div className="py-12 text-center text-gray-400">
                <MapPin className="w-8 h-8 mx-auto mb-2 opacity-40 text-gray-400" />
                <p className="text-xs font-semibold">No countries found matching "{searchQuery}"</p>
                <p className="text-[11px] text-gray-400 mt-1">Try searching Japan, New Zealand, Fiji, Australia, or United States</p>
              </div>
            ) : (
              countryResults.map((country, idx) => {
                const isSelected = country.isSingleTimezone
                  ? selectedTimezone === country.primaryTimezone
                  : country.timezones.includes(selectedTimezone);
                const currentTimeStr = formatTimeInTimezone(new Date(), country.primaryTimezone);
                const badgeText = country.iso3;
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 pr-2">
                      ...
                      <div className="min-w-0 truncate">
                        ...
                        <div className="text-[11px] text-slate-500 truncate mt-0.5">
                          {country.isSingleTimezone ? (
                            <span>{country.name}</span>
                          ) : (
                            <span>
                              {country.cities.length} regions • {country.previewCities.join(', ')}
                            </span>
                          )}
                        </div>
                      </div>
